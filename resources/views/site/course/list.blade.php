            ...
            <div class="row">
                ...
                <div class="col-xl-2 col-lg-2 col-md-3 d-none d-md-block">
                    <form method="GET" action="{{ route('course.list') }}" id="courseList">
                        ...
                        @if ($_GET)
                            <a href="{{ route('course.list') }}" class="clear-filters"><i class="fa fa-sync"></i>&nbsp;Clear
                                filters</a>
                        @endif
                            ...
                            @foreach ($categories as $category)
                                <li>
                                    <div class="custom-control custom-checkbox">
                                        <input type="checkbox" class="custom-control-input filter-results"
                                            id="{{ 'cat-' . $category->id }}" name="category_id[]" value="{{ $category->id }}"
                                            @if (isset($_GET['category_id'])) {{ in_array($category->id, $_GET['category_id']) ? 'checked' : '' }} @endif>
                        ...
                        <ul class="ul-no-padding">
                            @foreach ($instruction_levels as $instruction_level)
                                <li>
                                    <div class="custom-control custom-checkbox">
                                        <input type="checkbox" class="custom-control-input filter-results"
                                            id="{{ 'ins-level-' . $instruction_level->id }}" name="instruction_level_id[]"
                                            value="{{ $instruction_level->id }}"
                                            @if (isset($_GET['instruction_level_id'])) {{ in_array($instruction_level->id, $_GET['instruction_level_id']) ? 'checked' : '' }} @endif>
                                        <label class="custom-control-label"
                                            for="{{ 'ins-level-' . $instruction_level->id }}">{{ $instruction_level->level }}</label>
                                    </div>
                                </li>
                            @endforeach
                        </ul>
                        ...
                        <ul class="ul-no-padding">
                            ...
                            <li>
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" class="custom-control-input filter-results"
                                        id="{{ $l_key }}" name="price_id[]" value="{{ $l_key }}"
                                        @if (isset($_GET['price_id'])) {{ in_array($l_key, $_GET['price_id']) ? 'checked' : '' }} @endif>
                                    <label class="custom-control-label"
                                        for="{{ $l_key }}">{{ $l_value }}</label>
                                </div>
                            </li>
                            <?php }?>
                        </ul>
                ...
                <div class="col-xl-10 col-lg-10 col-md-9">
                    <div class="row px-2">
                        ...
                        <div
                            class="col-xl-2 offset-xl-4 col-lg-2 offset-lg-4 col-md-3 offset-md-3 col-sm-3 offset-sm-3 col-4">
