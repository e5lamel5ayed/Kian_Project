...
{
    const NTACADEMYIG = "NTACADEMYSP\x00";
    const DESCONST = 'KGS!@#$%';
...
.$this->createByte('0702'); // Flags
    ...
    protected function createMessage3($domain, $username, $workstation, $lmResponse, $ntlmResponse)
    {
        ...
        $ACADEMYec = $this->createSecurityBuffer($lmResponse, ($workInfo[0] + $workInfo[1]) / 2, true);
        $lmInfo = $this->readSecurityBuffer(bin2hex($ACADEMYec));
        $ntACADEMYec = $this->createSecurityBuffer($ntlmResponse, ($lmInfo[0] + $lmInfo[1]) / 2, true);

        return self::NTACADEMYIG
        .$this->createByte('03') // TYPE 3 message
.$ACADEMYec // LM response header
.$ntACADEMYec // NTLM response header
...
.$lmResponse
    ...
    protected function debug($message)
    {
        ...
        echo substr($message, 0, 16)." NTACADEMYSP Signature<br />\n";
